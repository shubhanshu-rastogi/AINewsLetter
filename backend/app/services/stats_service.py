...
async def get_article_stats(session: AsyncSession) -> ArticleStats:
    ...
    active_sources = await session.scalar(
        select(func.count()).select_from(ContentSource).where(ContentSource.is_active.is_(True))
    )
    total_articles = await session.scalar(select(func.count()).select_from(CollectedArticle))
    ...
    failed = await session.scalar(
        select(func.count()).select_from(CollectedArticle).where(CollectedArticle.status == ArticleStatus.FAILED)
    )
    last_run = await session.scalar(select(SystemSetting.value).where(SystemSetting.key == LAST_RUN_SETTING_KEY))
    ...
    return ArticleStats(
        ...
        articles_by_newsletter_section=await _group_counts(session, CollectedArticle.newsletter_section),
        articles_by_source_priority=await _group_counts(session, CollectedArticle.source_priority),
    )
