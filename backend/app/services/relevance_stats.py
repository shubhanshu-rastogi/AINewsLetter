...
async def get_relevance_stats(session: AsyncSession) -> dict:
    ...
    duplicates = await session.scalar(
        select(func.count())
        .select_from(CollectedArticle)
        .where(CollectedArticle.status == ArticleStatus.DUPLICATE)
    )
    ...
    avg_score = await session.scalar(
        select(func.avg(CollectedArticle.overall_relevance_score))
    )
    ...
    return {
        ...
        "articles_by_section": await _group_counts(
        ...
        "top_sources": {
            (k or "uncategorized"): c for k, c in top_sources_rows
        },
    }
...
async def get_categories_distribution(session: AsyncSession) -> dict:
    return {
        ...
        "by_primary_category": await _group_counts(
            session, CollectedArticle.primary_category
        ),
    }
