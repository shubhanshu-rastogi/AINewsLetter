...
|---|---|---|---|
| 1 | ~~**`issue_number` race** under concurrent newsletter generation (unique-constraint collisions)~~ | Failed generations when run in parallel | **Fixed**: `_resolve_newsletter` retries allocation on `IntegrityError` (up to 5×) so concurrent generations no longer collide. Documented in `performance_report.md` |
...
| 4 | ~~**Publish/subscriber endpoints unauthenticated**~~ | Unauthorized publish/subscribe calls | **Fixed**: `publish_router`, `publications_router`, and the subscribers router now require `require_reviewer`. Enforced when `REVIEW_AUTH_TOKEN` is set (permissive otherwise for local/dev) |
